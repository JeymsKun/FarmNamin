...
const MarketScreen = () => {
  ...
  const getProductsForCategory = (categoryValue) => {
    const category = marketData.categories.find(
  ...
  const handleSearch = () => {
    ...
    setTimeout(() => {
      const filtered = Object.entries(searchData).reduce(
        (acc, [title, types]) => {
    ...
    >
      ...
      <View style={styles.header}>
        <View style={styles.headerTitle}>
          <Text style={styles.headerTitleText}>
            Good Day, {user?.first_name.trim() || "User"}!
          </Text>
        </View>
        <View style={styles.headerGreet}>
          <Text style={styles.headerTitleText}>
            Ready to check the latest prices for your goods?
          </Text>
        </View>
      </View>
      ...
      <View style={styles.marketPriceContainer}>
        <View style={styles.rowMarket}>
          <Text style={styles.titleMarket}>
            Agriculture and Fishery Market Price
          </Text>
        </View>
      </View>
          ...
          {selectedCategory ? (
            getProductsForCategory(selectedCategory).map((product, index) => (
              <View key={index} style={styles.productRow}>
                <Text
                  style={[
                    ...
                    product.name.length >= 8
                      ? styles.productTextSmall
                      : styles.productTextNormal,
                  ]}
                >
                  {product.name}
                </Text>
                <View style={styles.currentPriceContainer}>
                  <Ionicons
                    name={
                      product.prevailingPrice >= product.highPrice
                        ? "arrow-up"
                        : "arrow-down"
                    }
                    size={16}
                  ...
                  <Text style={styles.productTextPrice}>
                    ₱ {product.prevailingPrice.toFixed(2)}
                  </Text>
                </View>
                <View style={styles.highLowContainer}>
                  <Ionicons name="arrow-up" size={14} color="green" />
                  <Text style={styles.productTextPriceHighLow}>
            ...
            <View style={styles.selectText}>
              <Text style={styles.select}>
                Please select a category to see the products and prices.
              </Text>
            </View>
          )}
        </View>
      ...
      <View style={styles.marketContainer}>
        <View style={styles.searchBarContainer}>
          <Text style={styles.searchTitle}>
            2025 Price Estimates for Key Agricultural Products: What to Expect
          </Text>
            ...
            <View style={styles.productWrapper}>
              {item.types.map((type, idx) => (
                <View key={idx} style={styles.productTypeContainer}>
                  <Text style={styles.productType}>{type.type}:</Text>
        ...
        {!loading && noResults && filteredSuggestions.length === 0 && (
          <View style={styles.noResultContainer}>
            <Text style={styles.noResultsText}>
              No Result? Let Us Know What You Need 😊
            </Text>
      ...
      <View style={styles.informationContainer}>
        ...
        <View style={styles.wrapperInformation}>
          <BulletLink
            text="Price Monitoring | DA Region 10 "
            stylePagasa={{ fontSize: 14 }}
            url="https://cagayandeoro.da.gov.ph/?page_id=55148"
          />
        </View>
        <View style={styles.wrapperInformation}>
          <BulletLink
            text="PH | Retail Price: Selected Agricultural Commodities | CEIC "
            stylePagasa={{ fontSize: 11 }}
            url="https://www.ceicdata.com/en/philippines/retail-price-selected-agricultural-commodities?page=2"
          />
        </View>
        <View style={styles.wrapperInformation}>
          <BulletLink
            text="Bantay Presyo"
            stylePagasa={{ fontSize: 14 }}
            url="http://www.bantaypresyo.da.gov.ph/tbl_veg.php"
          />
        </View>
      </View>
    </ScrollView>
