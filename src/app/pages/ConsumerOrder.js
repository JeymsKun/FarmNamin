...
import markProductAsDone from "../utils/markProductAsDone";
...
const ConsumerOrderPage = ({ route }) => {
  ...
  const handleProceedOrder = async () => {
    if (isChecked) {
      ...
      const currentAvailable = availableMatch
      ...
      if (error) {
        showAlert(
          "Order Error",
          "Failed to record the order. Please try again."
        );
      ...
      const nonNumericPart = availableString
        .replace(/(\d+(\.\d+)?)/, "")
        .trim();
      const updatedAvailableString = `${newAvailable.toFixed(
        2
      )} ${nonNumericPart}`;
      ...
      if (updateError) {
        showAlert(
          "Update Error",
          "Failed to update available quantity. Please try again."
        );
      ...
      console.log(
        "Available quantity updated successfully. New available quantity:",
        updatedAvailableString
      );
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <Text style={styles.modalText}>
              Farmers may not always be online. Please contact the farmer within
              3 days to confirm the order. Unconfirmed orders will be
