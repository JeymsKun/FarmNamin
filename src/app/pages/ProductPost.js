...
export default function Product() {
  ...
  const [showInfoMessageAdditional, setShowInfoMessageAdditional] =
    useState(false);
  ...
  const handleDeleteMediaItem = (index) => {
    setImagesVideo((prevImagesVideo) =>
      prevImagesVideo.filter((item, i) => i !== index)
    );
  };

  const handleDone = () => {
    if (
      imagesVideo.length === 0 ||
      name.trim() === "" ||
      productPrice.trim() === "" ||
    ...
    try {
      await Promise.all(
        imagesVideo
          ...
          .map((image) => handleUploadFileToSupabase(image.uri, "image"))
      );

      await Promise.all(
        imagesVideo
          .filter((item) => item.type === "video")
          .map((video) => handleUploadFileToSupabase(video.uri, "video"))
      );
      ...
      const additionalDetailsData = {
        ...
        ideal_storage_temperature:
          detailValues["Ideal Storage Temperature"] || "",
        packaging_type: detailValues["Packaging Type"] || "",
        community_support_projects:
          detailValues["Community Support Projects"] || "",
        ...
        special_handling_instructions:
          detailValues["Special Handling Instructions"] || "",
        farm_history: detailValues["Farm History"] || "",
        use_of_indigenous_knowledge:
          detailValues["Use of Indigenous Knowledge"] || "",
        use_of_technology_in_farming:
          detailValues["Use of Technology in Farming"] || "",
      };
  ...
  const handlePressMediaItem = (item) => {
    if (item.type === "video") {
      ...
      router.push("/support/VideoPlayer");
        ...
        >
          ...
          <View style={styles.productContainer}>
            <View style={styles.actionButtons}>
              <Text style={styles.sectionTitlePost}>
          ...
          <View style={styles.inputContainer}>
            ...
            <View
              style={[
                styles.inputWrapper,
                focusedName && name.length === 0 && styles.errorBorder,
              ]}
            >
              <TextInput
                style={styles.input}
            ...
            <Text style={styles.inputTitlesCategoryNote}>
              *Note: The unit price should be the same as the product price.
            </Text>
            <View
              style={[
                styles.inputWrapper,
                focusedProductUnitPrice &&
                  productPrice.length === 0 &&
                  styles.errorBorder,
              ]}
            >
            ...
            <Text style={styles.inputTitlesCategory}>
              Category:{" "}
              <Text style={styles.inputTitlesCategoryNote}>
                *Note: Feel free to create your own categories.
              </Text>
            </Text>
            <View
              style={[
                styles.inputWrapperCategory,
                focusedCategory && category.length === 0 && styles.errorBorder,
              ]}
            >
              <TextInput
            ...
            <View style={styles.additionalDetailsContainer}>
              ...
              <TouchableOpacity
                ...
                onPress={() => setShowInfoMessageAdditional((prev) => !prev)}
              >
                <AntDesign
                  name="questioncircleo"
                  size={12}
              ...
              <TouchableOpacity
                style={styles.additionalButton}
                onPress={() =>
                  router.push("/pages/AdditionalDetails", {
                    selectedOptions: additionalDetails,
                  })
                }
              >
              ...
              >
                <Animated.View
                  style={{ transform: [{ rotate: xIconRotationInterpolate }] }}
                >
                  <Feather
                    name={deleteMode ? "x-circle" : "minus-circle"}
                    size={22}
                    color="black"
                  />
                </Animated.View>
              ...
              {showInfoMessageAdditional && (
                <View style={styles.infoMessageAdditional}>
                  <Text style={styles.infoTextAdditional}>
                    This section allows you to provide additional details about
                    your product. You can include information such as:
                  </Text>
                  <Text style={styles.bulletPoint}>
                    {`\u2022`} Where the product was made?
                  </Text>
                  <Text style={styles.bulletPoint}>
                    {`\u2022`} When it was made or harvested?
                  </Text>
                  <Text style={styles.bulletPoint}>
                    {`\u2022`} How it was produced or manufactured?
                  </Text>
                  <Text style={styles.bulletPoint}>
                    {`\u2022`} Who made or harvested the product?
                  </Text>
                  <Text style={styles.bulletPoint}>
                    {`\u2022`} Any special process or unique characteristics.
                  </Text>
                  <Text style={styles.bulletPoint}>
                    {`\u2022`} And other specifics you want to include.
                  </Text>
                </View>
              )}
            </View>
            ...
            {additionalDetails.map((detail, index) => (
              <View key={index}>
                ...
                <View
                  style={[
                    styles.inputWrapper,
                    focusedDetails[detail] &&
                      detailValues[detail]?.length === 0 &&
                      styles.errorBorder,
                  ]}
                >
                  <TextInput
                    ...
                    onBlur={() =>
                      setFocusedDetails((prev) => ({ ...prev, [detail]: true }))
                    }
                  />
                  {deleteMode && (
                    <TouchableOpacity
            ...
            {showNoDetailsMessage && (
              <Text style={styles.noDetailsMessage}>
                You have no details added. You can click the plus sign to add
                details.
              </Text>
            )}
          ...
          >
            <View style={styles.modalContainer}>
              <View style={styles.modalContent}>
                <Text style={styles.modalTitle}>
                  Are you sure you want to post your product?
                </Text>
                <View style={styles.modalButtons}>
                  <TouchableOpacity
                    style={styles.modalButton}
          ...
          <Modal visible={isLoading} transparent={true} animationType="fade">
            ...
            >
              <View
                style={{ padding: 20, borderRadius: 10, alignItems: "center" }}
              >
