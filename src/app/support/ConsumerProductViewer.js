...
const ConsumerProductViewer = ({ route, navigation }) => {
  ...
  const { setSelectedFarmer, setSelectedImageUri } = useGlobalState();
  ...
  useEffect(() => {
    if (favoriteProducts) {
      const isFavorite = favoriteProducts.some(
        (fav) => fav.product.id === product.id && fav.is_bookmarked
      );
  ...
  const handleFeedbackSubmit = async (farmerId) => {
    ...
    if (user.role !== "consumer") {
      showAlert(
        ...
        "Only consumers can submit feedback."
      );
      return;
    }
            ...
            <View style={styles.farmerInfoContainer}>
                    ...
                    <View style={styles.nameContainer}>
                      <Text style={styles.name}>
                        {`${(farmer.first_name || "").trim()} ${(
                          farmer.middle_name || ""
                        ).trim()} ${(farmer.last_name || "").trim()}${
                          farmer.suffix ? `, ${farmer.suffix.trim()}` : ""
                        }`}
                      </Text>

                      <Text style={styles.farmerContactNumText}>
                        {formatPhoneNumberForDisplay(farmer.phone_number) ||
              ...
              <View style={styles.wrapContainer}>
                <TouchableOpacity
                  ...
                  onPress={showOrderInput ? handleCancelOrder : handleOrderNow}
                >
                  {isLoading ? (
                    <ActivityIndicator size={24} color="white" />
                  ) : (
                    <Text style={styles.orderText}>
                      {showOrderInput ? "Cancel Order" : "Order Now"}
                    </Text>
                  )}
                </TouchableOpacity>
              </View>
                ...
                <View style={styles.orderInputContainer}>
                  {(() => {
                    const availableMatch =
                      product.available.match(/(\d+(\.\d+)?)/);
                    const availableQuantity = availableMatch
                      ? parseFloat(availableMatch[1])
                      : 0;
                    const adjustedAvailable = quantity
                      ? (availableQuantity - parseFloat(quantity)).toFixed(2)
                      : availableQuantity.toFixed(2);
                    const displayAvailable =
                      adjustedAvailable < 0 ? "0.00" : adjustedAvailable;
                  ...
                  <Text style={styles.orderInputLabel}>
                    Unit Price: {formatUnitPrice(product.unit_price)}
                  </Text>
                  <Text style={styles.orderInputLabel}>
                    Total:{" "}
                    {formatPrice(
              ...
              {farmersData.map((farmer) => (
                <View key={farmer.id_user} style={styles.feedbackWrapper}>
                  <Text style={styles.farmerTitle}>
                    Feedback for {farmer.first_name}
                  </Text>
                  ...
                  <View style={styles.inputContainer}>
                    <Text style={styles.inputText}>
                      Share your tags that describe your experience.
                    </Text>
                    <Text style={styles.note}>
                      Note: Tags are limited to 5 words.
                    </Text>
                    <TextInput
